...
function load(file, options) {
  ...
  var module = {
    ...
    growMemory: function growMemory(numPages){
      var previousPages = this.memory.grow(numPages);
      onGrowMemory();
  ...
  module.array = {
    ...
    create: function(value, elementByteSize) {
      var ptr = (module.exports.malloc || imports.env.malloc)(4 + value.length * elementByteSize);
      return {
        ptr: ptr,
        base: ptr + 4
      };
  ...
  function onGrowMemory() {
    buffer = module.buffer = new Uint8Array(memory.buffer);
  }
